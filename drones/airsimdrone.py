...
class AirSimDrone(Drone):
	...
	def move(self, x_rel, y_rel, z_rel, speed):
		...
		self._airsim._client.moveToPositionAsync(target_position[0], target_position[1], target_position[2], speed).join()
	...
	def rotate(self, yaw_deg):
		...
		self._airsim._client.rotateToYawAsync(target_yaw).join()
