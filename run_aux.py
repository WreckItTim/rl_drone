...
## test_case vars
use_slim = use_res = vert_motion = False
if test_case in ['h4', 'pyro', 'phoenix', 'ml19']:
	use_slim = True
if test_case in ['h4', 'torch', 'phoenix', 'ml21']:
	use_res = True
if test_case in ['h4', 'pyro', 'torch']:
	vert_motion = True
if use_slim and use_res:
	child_project = 'NaviFuse'
elif use_slim:
	child_project = 'NaviSlim'
elif use_res:
...
if use_res:
	aux_actions.append('FlattenedDepthResolution1')
...
if use_slim:
	rewards['SlimReward']= 3/reward_norm
if use_res:
	rewards['ResolutionReward1']= 0.5/reward_norm
	rewards['ResolutionReward2']= 0.5/reward_norm
rewards['MaxStepsReward'] = 0/reward_norm
...
controller_params = {
	...
	'train_environment_component' : 'AuxTrainEnvironment',
...
def create_base_components(
	...
	# NEW CONFIGURATION?
		...
		if 'MoveForward' in navi_actions:
		...
		if 'MoveVertical' in navi_actions:
			from actions.move import Move 
			Move(
				drone_component = 'Drone', 
		...
		if 'FlattenedDepthResolution1' in aux_actions:
		...
		if 'FlattenedDepthResolution2' in aux_actions:
			from actions.resolution import Resolution 
			Resolution(
				scales_components = [
					'ResizeFlat2',
		...
		if vert_motion:
			motion = 'vertical'
		Spawn(
			read_path='aPaths_' + motion + '_all.p', # read in dict of possible paths or static spawns
		...
		Curriculum(
			models_directory=working_directory+'AuxModel/',
			model_component='AuxModel',
