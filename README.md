...
step 6: run the python file run_train.py to run an example reinforcement learning setup on the Blocks release. This will use a CNN feature extractor to capture depths from the quadcopter camera, using an Atari CNN architecture with randomized weights. Drone dynamics will be fed into an MLP that is added onto the feature extractor. The MLP is then used to decide which actions to take based on the current state. The neural network is trained using a TD3 reinforcement learning algorithm. It is trained to get from objective A to B without colliding. At any time run the train_eval.py file to evaluate the progress of the reinforcement learning algorithm. See paper for more implementation details.
